...
## Annotation Hub Enterprise 2.8.0
...
- Added a new `disable_reviewer_annotator_tokens`setting for organization licenses. When set to `true`, token authorization is disabled for users in the Reviewer and Annotator role. This will help maintain organization security by preventing users from accessing Annotation Hub via the API.

- Updated the font and spacing for the **Submit and Exit** button to match Annotation Hub UI styling guidelines.

- Updated text on the SAML and SCIM settings pages to match Annotation Hub UI styling guidelines.
...
- Due to an XSS vulnerability, we previously added a requirement that users must log in to view the Annotation Hub API doc reference and Swagger. You can now view the [API docs](https://app.heartex.com/docs/api/) without logging in. However, the Swagger version is still only available to logged in users. 
