---
title: Set up webhooks in Annotation Hub
short: Webhooks
type: guide
tier: all
order: 407
order_enterprise: 407
meta_title: Configure Webhooks in Annotation Hub
meta_description: Annotation Hub documentation for setting up and configuring webhooks to integrate Annotation Hub with your machine learning pipeline.
section: "Integrate & Extend"

---

Webhooks in Annotation Hub let you set up integrations that subscribe to certain events that occur inside Annotation Hub. When an event is triggered, Annotation Hub sends an HTTP POST request to the configured webhook URL. For example:
```json
{
    "action": "TASK_CREATED",
    "tasks": [
...
## What to use Annotation Hub webhooks for 

Some events that happen in Annotation Hub happen in response to an API call. For those, the API response is sufficient to know that the action has occurred. But other events happen inside the Annotation Hub UI, such as project changes and task annotation activities. To find out immediately when those changes occur, you can use webhooks to notify your application or machine learning pipeline to take specific actions. 
...
## Available Annotation Hub webhooks
Annotation Hub makes two main types of events available to integrate with webhooks: project-level task events and organization events.
...
To integrate with webhooks in Annotation Hub, you must do the following:
1. Create a webhook endpoint or URL as part of your application or machine learning model pipeline.
2. Make sure the webhook endpoint or URL is accessible to your Annotation Hub instance and can accept HTTP POST requests.
3. [Add the webhook to Annotation Hub](webhooks.html#Add-a-new-webhook-in-Label-Studio).

Set up your webhook endpoint to read the event payload from Annotation Hub and take action based on the event.

## Add a new webhook in Annotation Hub

Set up a webhook URL in Annotation Hub and associate it with one or more event triggers. You can set up a webhook URL in the Annotation Hub UI, or using the API. You can configure as many webhook connections as you want, but too many might overwhelm your instance. 

### Add a webhook in Annotation Hub UI

Add a webhook URL to Annotation Hub. The webhook URL must be set up to accept HTTP POST requests.

1. In the Annotation Hub UI, open the project that you want to associate with a webhook URL.
...
8. (Optional) Select whether to send an event for all actions in Annotation Hub supported by webhooks, or specific events. By default, events are sent for all actions. For example, select the `Annotation created` event. 
9. Save the webhook.

### Add a webhook using the Annotation Hub API
...
If the webhook URL is inaccessible by Annotation Hub, you can see this in a traceback in the logs. 

Annotation Hub does not retry webhook connections that fail. You can see successful webhook deliveries in the logs in DEBUG mode. 
