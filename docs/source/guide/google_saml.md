...
short: Google SAML SSO with Annotation Hub Setup Example
tier: enterprise
order: 253
meta_title: Google SAML SSO with Annotation Hub Setup Example
meta_description: Annotation Hub Enterprise documentation for setting up Google as Identity Provider for SAML SSO Setup Example.
hide_sidebar: true
---

You can set up Annotation Hub to use Google's IdP for SSO. 
...
* Your Annotation Hub user role must be Owner or Administrator. 


## Step 1: Get Annotation Hub ACS endpoints as a Service Provider (SP)

1. **Access Annotation Hub SSO settings:**
   - Log in to Annotation Hub with an account that has the Owner or Administrator role.
...
2. **Add a new SAML app:**
   ...
   - Enter a name for the app (e.g. "Annotation Hub") and click **Continue**.

    <img src="/images/google-saml/2-add-app.png" class="gif-border">

3. **Download IdP metadata:**
   - When given the option, download the IdP metadata file for your new app. This file contains all the necessary information to configure Annotation Hub.
...
4. **Configure SAML settings:**
   - In the **Service Provider Details** section, enter the following:
     - **ACS URL**: Paste the ACS URL copied from Annotation Hub.
     - **Entity ID**: Use the same URL as the ACS URL.
     - **Start URL**: (Optional) You can leave this blank or use the **Login URL** from Annotation Hub.
...
## Step 4: Complete configuration in Annotation Hub

1. **Upload IdP Metadata:**
   - Return to the Annotation Hub SSO & SAML settings page.
...
1. **Return to the Google Admin console**:
   ...
   - Select the Annotation Hub app.

    <img src="/images/google-saml/12-assign-group-1.png" class="gif-border">

2. **Assign the Annotation Hub application to the group:**
...
**Log in using SSO:**
   - Navigate to the Annotation Hub login page.
   - Click the **SSO Login** button and enter your company domain if prompted.
   - You should be redirected to the Google SSO login page. After successful authentication, you will be redirected back to Annotation Hub.

<img src="/images/google-saml/15-lse-sso-login.png" class="gif-border">

## Additional notes

- If you encounter any issues, verify that the SAML attributes and URLs are correctly configured in both Annotation Hub and Google Admin Console.

For more detailed information, you can refer to the
- [Annotation Hub SSO setup guide](auth_setup)
