...
meta_description: Reference documentation for implementing the Annotation Hub frontend into your own machine learning or data science application workflows.
section: "Integrate & Extend"
---

Annotation Hub frontend (LSF) includes several UI options and callbacks that you can use when implementing the frontend with a custom labeling backend, or when customizing the Annotation Hub interface.
...
!!! warning
    LSF version 1.0.0 is not compatible with earlier versions of Annotation Hub.
...
If you rely on specific formatting of Annotation Hub completed tasks, [Annotation Hub's annotation format](export.html#Raw-JSON-format-of-completed-tasks) has also been updated.

## Implement the Annotation Hub Frontend

```javascript
var labelStudio = new LabelStudio("editor", options);
```

The following options are recognized when initializing a Annotation Hub instance version 1.0.0.
...
- `topbar` - A labeling interface that lists the top-level items in the Annotation Hub UI.
- `instruction` - A button for the [instructions](#description).
- `side-column` - Show a column on the left or right side of the Annotation Hub UI.
...
LSF has a built-in event system that allows you to listen to events and trigger custom actions. You can subscribe to or unsubscribe from event at any time after the Annotation Hub instance is initialized.
...
Annotation Hub instance is loaded. 
...
| ------------- | -------- | ------------------------ |
| `labelStudio` | `Object` | Instance of Annotation Hub |
...
| Argument         | Type      | Description                      |
| ---------------- | --------- | -------------------------------- |
| `store`          | `Object`  | Instance of Annotation Hub         |
| `labelStudio`    | `Object`  | Instance of Annotation Hub         |
| `params`         | `Object`  |                                  |
...
Called when Annotation Hub has fully loaded and is ready for labeling. `ls` is the label studio instance
