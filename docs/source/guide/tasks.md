...
\+ *Annotation Hub Enterprise and Starter Cloud only*
...
You can place multiple tasks in one JSON file if you're uploading the JSON file using Annotation Hub Import Dialog only (Data Manager => Import button), or when importing from [cloud storage](storage.html). When using cloud storage, you must ensure every task in the file is formatted the same way. 
