...
When a user interacts with the model, you can import the user prompt and the model response into Annotation Hub and then use a labeling interface designed for a response moderation task.
...
To create evaluation task from LLM response and import it into the created Annotation Hub project, you can use the format in the following example:
...
# the task to import into Annotation Hub
task = {'chat': messages}
```
