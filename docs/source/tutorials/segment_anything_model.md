...
meta_title: Interactive annotation in Annotation Hub with Segment Anything Model (SAM)
meta_description: Annotation Hub tutorial for labeling images with MobileSAM or ONNX SAM. 
categories:
    - Computer Vision
    - Object Detection
    - Image Annotation
...
# Interactive annotation in Annotation Hub with Segment Anything Model

https://github.com/shondle/label-studio-ml-backend/assets/106922533/42a8a535-167c-404a-96bd-c2e2382df99a

Use Facebook's Segment Anything Model with Annotation Hub!
...
The Annotation Hub SAM backend works best if you have [Local
...
### Setting up the Annotation Hub server
...
For example, if you're launching Annotation Hub with Docker, you can enable these variables with
...
Note the IP address that you are running your Annotation Hub instance as the
`LABEL_STUDIO_HOST`. This will be necessary for setting up the connection to your
SAM model. 

_Because you are hosting both Annotation Hub and the ML backend in
...
Log into the Annotation Hub interface (in the example above, at
...
installation. Be sure to append the port that Annotation Hub is running on in
your `LABEL_STUDIO_HOST` variable, for example `http://192.168.1.36:8080` if
Annotation Hub is running on port 8080.
...
* `LABEL_STUDIO_ACCESS_TOKEN` sets the API access token for the Annotation Hub host.
...
## Set up a project in Annotation Hub for Segment Anything

Log into your Annotation Hub instance and perform the following steps.
...
4. Enter a title for the model, and the URL for the instance of the model you
   ...
   If you're running Annotation Hub in Docker or on another host, you
