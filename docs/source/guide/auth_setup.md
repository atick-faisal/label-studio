---
title: Set up SSO authentication for Annotation Hub
short: SSO and SAML
tier: enterprise
type: guide
order: 0
order_enterprise: 384
meta_title: SSO authentication for Annotation Hub Enterprise
meta_description: Annotation Hub Enterprise documentation for setting up SSO authentication for your data labeling, machine learning, and data science projects.
section: "Manage Your Organization"

---

Set up single sign-on using SAML to manage access to Annotation Hub using your existing Identity Provider (IdP).

!!! error Enterprise
    SSO authentication is only available in Annotation Hub Enterprise Edition. If you're using Annotation Hub Community Edition, see <a href="https://labelstud.io/guide/label_studio_compare.html">Annotation Hub Features</a> to learn more.


To more easily [manage access to Annotation Hub Enterprise](manage_users.html), you can map SAML groups to both `roles` or `workspaces`. 

## Set up SAML SSO

The organization Owner or Administrator for Annotation Hub Enterprise can set up SSO & SAML for the instance. Annotation Hub Enterprise supports the following IdPs:
...
After setting up the SSO, you can use native authentication to access the Annotation Hub UI, however it's not a recommended option especially for the user with the Owner role.
...
### Connect your Identity Provider to Annotation Hub Enterprise

Set up Annotation Hub Enterprise as a Service Provider (SP) with your Identity Provider (IdP) to use SAML authentication. 
...
4. Copy the following URLs:
    ...
    * **Login URL**---This is the URL that users will use to log in to Annotation Hub. 
    * **Logout URL**---This is the URL used to redirect users after successfully logging out of Annotation Hub.

###### From your IdP:

1. Paste the URLs copied from Annotation Hub in the appropriate location. 
2. Generate a metadata XML file, or a URL that specifies the metadata for the IdP.
3. Set up or confirm setup of the following SAML attributes. Annotation Hub Enterprise expects specific attribute mappings for user identities.
...
5. Test the configuration by logging in to Annotation Hub Enterprise with your SSO account.
...
If you want to manage Annotation Hub roles and workspaces entirely with single sign-on (SSO), add the following to your environment variable file:
...
Setting these options disables the Annotation Hub API and UI options to assign roles and workspaces for specific users within Annotation Hub and relies entirely on the settings in the environment variable file.

!!! info Tip
    If you are using the SaaS version of Annotation Hub (Annotation Hub Enterprise Cloud) and would like to enable these restrictions for your organization, [open a ticket](https://support.humansignal.com/hc/en-us/requests/new) to submit your request.  
