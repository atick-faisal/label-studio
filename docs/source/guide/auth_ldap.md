---
title: Set up LDAP authentication for Annotation Hub
short: LDAP
tier: enterprise
type: guide
order: 0
order_enterprise: 383
meta_title: LDAP authentication for Annotation Hub Enterprise
meta_description: Annotation Hub Enterprise documentation for setting up LDAP authentication for your data labeling, machine learning, and data science projects.
section: "Manage Your Organization"
date: 2024-05-24 09:08:58
---

Set up LDAP authentication to manage access to Annotation Hub.

!!! error Enterprise
    LDAP authentication is only available in Annotation Hub Enterprise Edition and is only available for on-prem installations. If you're using Annotation Hub Community Edition, see <a href="https://labelstud.io/guide/label_studio_compare.html">Annotation Hub Features</a> to learn more.


To more easily [manage access to Annotation Hub Enterprise](manage_users.html), you can map LDAP groups to both `roles` or `workspaces`. 

## Set up LDAP authentication 

After you set up LDAP authentication, you can no longer use native authentication to log in to the Annotation Hub UI. Set up LDAP authentication and assign LDAP users to your Annotation Hub Enterprise organization using environment variables in Docker. 

You can also map specific LDAP groups to specific organization roles and workspaces in Annotation Hub Enterprise, making it easier to set up and manage role-based access control (RBAC) and project access in Annotation Hub Enterprise. 
...
# Allow users to use usernames (not only emails) to log into Annotation Hub
...
# Query the authenticating user in Annotation Hub, it can be [email|username]
...
If you want to use a recursive scan to search in several LDAP groups to grant access to Annotation Hub, instead of relying on the simple search used by `AUTH_LDAP_USER_ON_TEMPLATE`, update your environment variables file like the following:
```bash
...
Map LDAP groups to specific Annotation Hub Enterprise roles using `;` to specify several groups.
...
Map LDAP groups to specific Annotation Hub workspaces. Use a JSON format where keys are workspace titles and values are LDAP groups. Split groups with `;` to specify several groups. 
...
If you want to manage Annotation Hub roles and workspaces entirely with LDAP, add the following to your environment variable file:
