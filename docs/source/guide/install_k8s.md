---
title: Deploy Annotation Hub on Kubernetes
short: Install using Kubernetes
tier: opensource
type: guide
order: 69
order_enterprise: 0
meta_title: Deploy Annotation Hub on Kubernetes
meta_description: Deploy Annotation Hub on Kubernetes, such as on Amazon Elastic Container Service for Kubernetes, to create machine learning and data science projects in a scalable containerized environment.
section: "Install & Setup"
parent: "install"
---

Deploy Annotation Hub on a Kubernetes Cluster using Helm 3. You can use this Helm chart to set up Annotation Hub for deployment onto a Kubernetes cluster and install, upgrade, and manage the application.
...
!!! warning
    To install Annotation Hub Enterprise Edition, see <a href="https://docs.humansignal.com/guide/install_enterprise_k8s">Deploy Annotation Hub Enterprise on Kubernetes</a>. This page is specific to the community version of Annotation Hub.

</div>

## Install Annotation Hub on Kubernetes

If you want to install Annotation Hub on Kubernetes and you have unrestricted access to the internet from your K8s cluster, follow these steps.
...
8. [Use Helm to install Annotation Hub on your Kubernetes cluster](#Use-Helm-to-install-Label-Studio-on-your-Kubernetes-cluster).

If you use a proxy to access the internet from your Kubernetes cluster, or it is airgapped from the internet, see how to [Install Annotation Hub without public internet access](/guide/install_k8s_airgapped.html).
...
Annotation Hub has the following default configurations for resource requests, resource limits, and replica counts:
...
To configure Annotation Hub to use TLS for end-client connections with PostgreSQL, do the following:
...
4. Install or upgrade Annotation Hub using Helm.

## Use Helm to install Annotation Hub on your Kubernetes cluster

Use Helm to install Annotation Hub on your Kubernetes cluster. Provide your custom resource definitions YAML file. Specify any environment variables that you need to set for your Annotation Hub installation using the `--set` argument with the `helm install` command.
...
## Restart Annotation Hub using Helm

Restart your Helm release by doing the following from the command line:

1. Identify the &lt;RELEASE_NAME&gt; of the latest Annotation Hub release:
```shell
helm list
```

2. Restart the Annotation Hub app:
```shell
kubectl rollout restart deployment/<RELEASE_NAME>-ls-app
```

## Upgrade Annotation Hub using Helm

To upgrade Annotation Hub using Helm, do the following.

1. Determine the latest tag version of Annotation Hub and add/replace the following in your `ls-values.yaml` file:
   ```yaml
   global:
     image:
...
## Uninstall Annotation Hub using Helm

To uninstall Annotation Hub using Helm, delete the configuration.
