---
title: Import pre-annotated data into Annotation Hub
short: Import pre-annotations
type: guide
tier: all
order: 163
order_enterprise: 163
meta_title: Import pre-annotated data into Annotation Hub
meta_description: Import predicted labels, predictions, pre-annotations, or pre-labels into Annotation Hub for your data labeling, machine learning, and data science projects.
section: "Import & Export"
parent: "tasks"
parent_enterprise: "tasks"
---

If you have predictions generated for your dataset from a model, either as pre-annotated tasks or pre-labeled tasks, you can import the predictions with your dataset into Annotation Hub for review and correction. Annotation Hub automatically displays the pre-annotations that you import on the Labeling page for each task. 

You can import pre-annotated tasks into Annotation Hub [using the UI](tasks.html#Import-data-from-the-Label-Studio-UI) or [using the API](/api#operation/projects_import_create). 

!!! note 
    To generate interactive pre-annotations with a machine learning model while labeling, see [Set up machine learning with Annotation Hub](ml.html).

<div class="opensource-only">

!!! error enterprise
    Annotation Hub Enterprise customers have access to a powerful tool called Prompts. With Prompts, you can leverage LLMs to automatically generate predictions for your Annotation Hub tasks. For more information, see [Prompts](https://docs.humansignal.com/guide/prompts_overview).
...
!!! info Tip
    With Prompts, you can leverage LLMs to automatically generate predictions for your Annotation Hub tasks. For more information, see [Prompts](prompts_overview).

</div>


## Prepare pre-annotations for Annotation Hub 

To import predicted labels into Annotation Hub, you must use the [Basic Annotation Hub JSON format](tasks.html#Basic-Label-Studio-JSON-format) and set up your tasks with the `predictions` JSON key. The Annotation Hub ML backend also outputs tasks in this format. Check this common video tutorial showing how to convert a submitted annotation to a prediction:
...
To format pre-annotations for Annotation Hub not represented in these examples, refer to the sample results JSON for the relevant object and control tags for your labeling configuration, such as the [Audio tag](/tags/audio.html) for audio classification tasks. Each tag must be represented in the JSON pre-annotations format to render predictions in the Annotation Hub UI. Not all object and control tags list sample results JSON. 

You can also use the [Annotation Hub Playground](/playground) to preview the output JSON for a specific labeling configuration.
...
For image pre-annotations, Annotation Hub expects the x, y, width, and height of image annotations to be provided in percentages of overall image dimension. See [Units for image annotations](predictions.html#Units_for_image_annotations) on this page for more about how to convert formats.
...
Save this example JSON as a file to import it into Annotation Hub, for example, `example_prediction_task.json`.
...
Import pre-annotated tasks into Annotation Hub [using the UI](tasks.html#Import-data-from-the-Label-Studio-UI) or [using the API](/api#operation/projects_import_create). 

In the Annotation Hub UI, the imported prediction for this task looks like the following: 
<img src="../images/predictions_loaded.png" alt="screenshot of the Annotation Hub UI showing an image of airplanes with bounding boxes covering each airplane." style="max-width: 600px;" class="gif-border">
...
Import pre-annotated tasks into Annotation Hub [using the UI](tasks.html#Import-data-from-the-Label-Studio-UI) or [using the API](/api#operation/projects_import_create). In the Annotation Hub UI, the imported prediction for the first task looks like the following:

<center><img src="../images/predictions_loaded_text.png" alt="screenshot of the Annotation Hub UI showing the text with highlighted text labels and prediction scores visible." style="max-width: 100%;"></center>
...
If you want to import pre-annotations for brush mask image segmentation using the [BrushLabels tag](/tags/brushlabels.html), you must convert the masks to RLE format first. The [Annotation Hub Converter](https://github.com/HumanSignal/label-studio-converter) package has some helper functions for this. See the following for common conversion cases and guidance.

Install Annotation Hub Converter:
```
...
For more assistance, review this [example code creating a Annotation Hub task with pre-annotations](https://github.com/HumanSignal/label-studio-converter/blob/master/tests/test_brush.py#L11) for brush labels.
...
!!! note 
    The image data in this example task references an uploaded file, identified by the source_filename assigned by Annotation Hub after uploading the image. The best way to reference image data is using presigned URLs for images stored in cloud storage, or absolute paths to image data stored in local storage and added to Annotation Hub by [syncing storage](storage.html). 
