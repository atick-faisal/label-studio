...
meta_title: Annotation Hub tutorial to run Hugging Face Large Language Model backend
meta_description: This tutorial explains how to run Hugging Face Large Language model backend in Annotation Hub. Hugging Face Large Language Model Backend is a machine learning backend designed to work with Annotation Hub, providing a custom model for text generation.
categories:
    - Generative AI
...
This machine learning backend is designed to work with Annotation Hub, providing a custom model for text generation. The model is based on the Hugging Face's transformers library and uses a pre-trained model.
...
## Annotation Hub XML labeling config

This ML backend is compatible with a Annotation Hub labeling configuration that uses a `<TextArea>` tag. Here is an example of a compatible labeling configuration:
...
When you open the task in Annotation Hub, the text box will show the generated text based on the prompt defined in `<Text>`. Be sure you include some instructions in prompt (for example, _"Summarize the following text: ..."_) to see the meaningful results.
