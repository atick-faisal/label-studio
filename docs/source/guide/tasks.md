---
title: Get data into Annotation Hub
short: Import data
type: guide
tier: all
order: 157
order_enterprise: 157
meta_title: Import Data into Annotation Hub
meta_description: Label and annotate data for your machine learning and data science projects using common file formats or the Annotation Hub JSON format.
section: "Import & Export"
---

Get data into Annotation Hub by importing files, referencing URLs, or syncing with cloud or database storage. 
...
- If your data is stored at internet-accessible URLs, in files, or directories, [import it from the Annotation Hub UI](#Import-data-from-the-Label-Studio-UI).
- If your data is stored locally, [import it into Annotation Hub](#Import-data-from-a-local-directory).
- If your data contains predictions or pre-annotations, see [Import pre-annotated data into Annotation Hub](predictions.html).
...
!!! attention 
    For large projects or business critical projects, do not [upload media files through the Annotation Hub interface](#Import-data-from-the-Label-Studio-UI). This is especially true for files such as images, audio, video, timeseries, etc.

    Uploading data through the Annotation Hub UI works fine for proof of concept projects, but it is not recommended for larger projects. Annotation Hub is not designed as a hosting service at scale and does not have backups for imported media resources. 
    ...
    **Risks when uploading through the UI**:<br />
    You will face challenges when attempting to do the following: 
        ...
        * Exporting your data
        * Moving your data to another Annotation Hub instance 
        * Redeploying Annotation Hub

    We ***strongly*** recommend that you configure [source storage](storage) instead.


## Types of data you can import into Annotation Hub
...
If you don't see a supported data or file type that you want to import, please let us know by submitting an issue to the <a className="no-go" href="https://github.com/humansignal/label-studio/issues">Annotation Hub Repository</a>.


### How to import your data

The most secure and reliable method to import your data is to store the data outside of Annotation Hub and import references to the data using URLs. You can import a list of URLs in a TXT, CSV, or TSV file, or reference the URLs in [JSON task format](#Basic-Label-Studio-JSON-format).
...
- To load data directly into the Annotation Hub database, specify `valueType="text"` for `HyperText` or `Text` data, or `valueType="json"` for `Paragraph` or `TimeSeries` data.

!!! note
    If you load data from a URL, the data is not saved in Annotation Hub. If you want an annotated task export to include the data that you annotated, you must import the data into the Annotation Hub database without using URL references, or combine the data with the annotations after exporting.
...
Use this parameter to retrieve data from multi-column csv on [S3 or other cloud storage](/guide/storage.html). Annotation Hub can retrieve it only in run-time, so it's secure.
...
Annotation Hub treats different file types different ways. 

If you want to import multiple types of data to label at the same time, for example, images with captions or audio recordings with transcripts, you must use the [basic Annotation Hub JSON format](#Basic-Label-Studio-JSON-format). 

[You can also use a CSV file or a JSON list of tasks to point to URLs with the data](#How-to-import-your-data), rather than directly importing the data if you need to import thousands of files. You can import files containing up to 250,000 tasks or up to 50MB in size into Annotation Hub.
...
### Basic Annotation Hub JSON format

The best way to import data into Annotation Hub is to use a JSON-formatted list of tasks. The `data` key of the JSON file references each task as an entry in a JSON dictionary. If there is no `data` key, Annotation Hub interprets the entire JSON file as one task. 
...
Depending on the type of object tag, Annotation Hub interprets field values differently:
...
| --- | --- | 
| annotations | Optional. List of annotations exported from Annotation Hub. [Annotation Hub's annotation format](export.html#Raw-JSON-format-of-completed-tasks) allows you to import annotation results in order to use them in subsequent labeling tasks. |
| predictions | Optional. List of model prediction results, where each result is saved using [Annotation Hub's prediction format](export.html#Raw-JSON-format-of-completed-tasks). Import predictions for automatic task pre-labeling and active learning. See [Import predicted labels into Annotation Hub](predictions.html) |
...
```
If you're placing JSON files in [cloud storage](storage.html), place 1 task in each JSON file in the storage bucket. If you want to upload a JSON file from your machine directly into Annotation Hub, you can place multiple tasks in one JSON file and import it using Annotation Hub GUI (Data Manager => Import button). 

#### Example JSON with multiple tasks
You can place multiple tasks in one JSON file if you're uploading the JSON file using Annotation Hub Import Dialog only (Data Manager => Import button). 
...
#### Example JSON for older versions of Annotation Hub
If you're still using a Annotation Hub version earlier than 1.0.0, refer to this example JSON format. 

<br/>
{% details <b>For versions of Annotation Hub earlier than 1.0.0, use this JSON format example.</b> %}
If you're using a version of Annotation Hub earlier than version 1.0.0, import tasks that match the following JSON format: 
...
When you import a CSV / TSV formatted text file, Annotation Hub interprets the column names are as task data keys that correspond to the labeling config you set up: 
```csv
...
!!! note
    If your labeling config has a `TimeSeries` tag, Annotation Hub interprets the CSV/TSV as time series data when you import it. This CSV/TSV is hosted as a resource file and Annotation Hub automatically creates a task with a link to the uploaded CSV/TSV.

### Plain text

Import data as plain text. Annotation Hub interprets each line in a plain text file as a separate data labeling task. 
...
- Run a web server to generate URLs for the files, then upload a file that references the URLs to Annotation Hub. 
- Add the file directory as a source or target [local storage](storage.html#Local-storage) connection in the Annotation Hub UI.

### Run a web server to generate URLs to local files

To run a web server to generate URLs for the files, you can refer to this provided [helper shell script in the Annotation Hub repository](https://github.com/HumanSignal/label-studio/blob/develop/scripts/serve_local_files.sh) or write your own script. 
Use that script to do the following:
1. On the machine with the file directory that you want Annotation Hub to import, call the helper script and specify a regex pattern to match the files that you want to import. In this example, the script identifies files with the JPG file extension:
   ```bash
...
3. Import the file with URLs into Annotation Hub using the Annotation Hub UI. 

!!! note 
    You must keep the web server running while you perform your data labeling so that the URLs remain accessible to Annotation Hub.

If your labeling configuration supports HyperText or multiple data types, use the Annotation Hub JSON format to specify the local file locations instead of a `txt` file. See [an example of this format](storage.html#Tasks-with-local-storage-file-references).

If you serve your data from an HTTP server created like follows: `python -m http.server 8081 -d`, you might need to set up CORS for that server so that Annotation Hub can access the data files successfully. If needed, run the following from the command line:
```bash
npm install http-server -g
http-server -p 3000 --cors
```

### Add the file directory as source storage in the Annotation Hub UI

If you're running Annotation Hub on Docker and want to add local file storage, you need to mount the file directory and set up environment variables. See [Run Annotation Hub on Docker and use local storage](https://labelstud.io/guide/start#Run-Label-Studio-on-Docker-and-use-Local-Storage).


## Import data from the Annotation Hub UI
...
To import data from the Annotation Hub UI, do the following:
1. On the Annotation Hub UI, open the Data Manager page for a specific project.
...
Import your data using the Annotation Hub API. See the [API documentation for importing tasks](/api#operation/api_projects_import_create).

### Import data from the command line

In versions of Annotation Hub earlier than 1.0.0, you can import data from a local directory using the command line. 

To import data from the command line, do the following:

1. Start Annotation Hub and use command line arguments to specify the path to the data and format of the data. <br/>For example: <br/>`label-studio init --input-path my_tasks.json --input-format json`
2. Open the Annotation Hub UI and confirm that your data was properly imported. 

You can use the `--input-path` argument to specify a file or directory with the data that you want to label. You can specify other data formats using the `--input-format` argument. For example run the following command to start Annotation Hub and import audio files from a local directory:
...
!!! warning 
    The `--allow-serving-local-files` argument is intended for use only with locally-running instances of Annotation Hub. Avoid using it for remote servers unless you are sure what you're doing.

By default, Annotation Hub expects JSON-formatted tasks using the [Basic Annotation Hub JSON format](tasks.html#Basic-Label-Studio-JSON-format). 

If you add more files to a local directory after Annotation Hub starts, you must restart Annotation Hub to import the tasks in the additional files.
