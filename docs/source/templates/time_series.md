...
meta_description: Template for labeling multivariate and simple time series data with Annotation Hub for your machine learning and data science projects.
---
...
```
The `valueType="url"` parameter means that Annotation Hub expects links to CSV files in JSON-formatted tasks. The `timeColumn` parameter specifies the column in your dataset to use as the X-axis for time. If you don't specify a `timeColumn`, Annotation Hub uses incremental integer values as the X-axis: `0, 1, 2, ...`.
...
Annotation Hub supports several input types for time series:
...
Then, create a JSON file that references a URL for the CSV file to upload to Annotation Hub:
```json
...
All tasks in Annotation Hub are stored in JSON and this is the native format for Annotation Hub. 

- `valueType="url"`

  When you use `valueType="url"` for TimeSeries tag and import a CSV file, Annotation Hub automatically creates a JSON task with the body like this example: 
...
- `valueType="json"`
  ...
  You can also use Annotation Hub JSON format directly by creating and import JSON structured like the following example, where each key specifies the time and channels: 
...
Annotators add labels to time series tasks. Annotation Hub represents each completed annotation with a JSON structure. Each annotation has a `result` field that looks like the following example for time series labeling projects:
