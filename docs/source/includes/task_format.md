## Annotation Hub JSON format of annotated tasks 

When you annotate data, Annotation Hub stores the output in JSON format. The raw JSON structure of each completed task uses the following example: 
...
| project | Identifier for a specific project in Annotation Hub. |
...
| annotations.completed_by | User ID of the user that created the annotation. Matches the list order of users on the People page on the Annotation Hub UI. See [Specifying annotators during import](#specifying-annotators-during-import) for import format options. |
...
When importing annotations to Annotation Hub, you can control which users are assigned as annotators by using the `completed_by` field within the annotation object:
...
Note that this applies when importing via the Annotation Hub UI, the API, or the SDK. 
