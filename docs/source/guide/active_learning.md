---
title: Set up active learning with Annotation Hub
short: Active learning loop
tier: enterprise
type: guide
order: 0
order_enterprise: 257
meta_title: Set up an active learning loop with Annotation Hub
meta_description: Set up an end-to-end active learning loop with Annotation Hub using the ML backend SDK and webhooks to perform model training and predictions and labeling.
section: "Machine Learning"

---

Follow this tutorial to set up an active learning loop with Annotation Hub. 

Use Annotation Hub Enterprise Edition to build an automated active learning loop with a machine learning model backend. If you use the open source Community Edition of Annotation Hub, you can manually sort tasks and retrieve predictions to mimic an active learning process. If you're using Annotation Hub Community Edition, see how to [manually manage your active learning loop](#Set-up-manual-active-learning).
...
Continuously train and review predictions from a connected machine learning model using Annotation Hub. 
...
2. [Connect the ML backend for getting predictions to Annotation Hub](#Connect-the-ML-backend-to-Label-Studio-for-active-learning).
...
As you label tasks, Annotation Hub sends webhook events to your machine learning backend and prompts it to retrain. As the model retrains, the predictions from the latest model version appear in Annotation Hub. 
...
## Connect the ML backend to Annotation Hub for active learning

Follow the steps to [connect a model to a Annotation Hub project](ml#Connect-the-model-to-Label-Studio) and ensure the setting **Start model training on annotation submission** is enabled. This sends a training request to the backend after each annotation submission or update. 

## Configure webhooks to send a training event to the ML backend (optional)

By default, Annotation Hub notifies your ML backend every time an annotation is created or updated so that it can start training in response. 
...
1. In the Annotation Hub UI, open the project that you want to use for active learning.
...
As your model retrains and a new version is updated in Annotation Hub, the tasks shown next to annotators are always those with the lowest prediction scores, reflecting those with the lowest model certainty. The predictions for the tasks correspond to the latest model version.
...
If you're using Annotation Hub Community Edition, data annotators can't experience a live active learning loop. You can mimic an active learning experience by doing the following:
