...
meta_description: Annotation Hub tutorial for OpenMMLab Image object detector or MMDetection
...
2. Run Annotation Hub, then go to the **Machine Learning** page in the project settings. Click **Add Model**, then paste the selected ML backend URL, by default `http://localhost:9090`.
...
## Run ML backend on the same machine as Annotation Hub

If you run the ML backend on the same machine as Annotation Hub, you can upload images directly to Annotation Hub or specify them as remote URLs. 

#### Images are uploaded in Annotation Hub UI
   ```bash
...
## Run ML backend server on the different machine as Annotation Hub

When running the ML backend on a separate server instance and connecting it to Annotation Hub with a remote hostname URL, you must remember that your ML backend server must get the full image URLs.
In this case, you must provide the Annotation Hub hostname before running ML backend:
